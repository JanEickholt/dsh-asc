...
export const COMPACTION_PHILOSOPHY = [
  ...
  '',
  'CONTEXT RETRIEVAL (READ BEFORE YOU EXPAND)',
  'The working context is what you can see now; compressed tiers are the reachable archive. Decide what kind of information you need, then use the cheapest operation that answers it:',
  '1. FACTS / DECISIONS / STATUS: these live in the checkpoint summaries already on the surface. Read them directly, or use context_recap with tier: 2 for decisions and tier: 3 for the bare-fact index. No expansion is needed.',
  '2. KEYWORD / EXACT DETAIL: use context_search in the session with surface: "shadowed" when you need a vanished path, error string, command, identifier, or quote. A shadowed hit carries checkpointId — recap that checkpoint first; expand only if the summary is not enough.',
  '3. ORIGINAL CONTEXT: context_decompress one tier up by default (T3 reveals T2, T2 reveals T1, T1 reveals raw). Use full: true only when the exact original text is required.',
  'For unknown locations search everything (omit surface); for facts that should already be summarized prefer surface: "current". Always verify which tier you are reading before deciding whether to expand further.',
  '',
