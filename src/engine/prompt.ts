...
export const COMPACTION_PHILOSOPHY = [
  ...
  '',
  'CONTEXT RETRIEVAL (LOCATE FIRST, EXPAND SECOND)',
  'Compressed context has a table of contents (surface checkpoints, topics, tiers) and a full-text index (context_search). Do not expand a checkpoint to discover what it contains; read its summary first.',
  'Two retrieval modes:',
  '- BLOCK NAVIGATION — when you know which work unit or phase you are looking for. Read the checkpoint list in context_status (topic/tier), or context_recap with tier: 2 for decisions and tier: 3 for facts. Once the right checkpoint is found, context_decompress one tier up if its summary is not enough.',
  '- TOKEN SEARCH — when you do not know which block owns the detail, but you can name an exact token (path, error string, command, identifier, quote). Use context_search; a shadowed hit names its checkpointId, and only then recap or decompress that checkpoint.',
  'Decision rule: name a work unit/topic → navigate the table of contents; name an exact token but not its location → search; name neither → read the tier-3/tier-2 summaries first to recover candidate terms, then navigate or search.',
  'Never search by expanding: context_decompress inflates the surface, so use it only after a summary or search hit has told you this checkpoint is the one you need. Search is the locator for unknown locations; decompress is the fetcher for a located block.',
  '',
